  ...
  app.get('/:id/finance/breakdown', companyScope, async (c) => {
    ...
    const result = await db.query<FinanceBreakdown>(
      `SELECT
         ...
         SUM(CASE WHEN event_type IN ('llm_call','tool_use') THEN amount_cents WHEN event_type = 'manual_adjustment' THEN amount_cents ELSE 0 END)::int AS total_cents,
  ...
  app.get('/:id/finance/top-spenders', companyScope, async (c) => {
    ...
    const result = await db.query<FinanceTopSpender>(
      `SELECT
         ...
         SUM(CASE WHEN fe.event_type IN ('llm_call','tool_use') THEN fe.amount_cents WHEN fe.event_type = 'manual_adjustment' THEN fe.amount_cents ELSE 0 END)::int AS total_cents,
         COUNT(*)::int AS event_count
