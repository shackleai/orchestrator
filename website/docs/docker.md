...
:::note How auto-init works
If no config exists on startup and `SHACKLEAI_COMPANY_NAME` is set, the orchestrator initializes automatically — no interactive `shackleai init` needed. If the company already exists in the database, it reuses the existing record.
:::
